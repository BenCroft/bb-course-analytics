...
```
```{r}
length(unique(sis_no_acad_plan$StudentID_Anon))
nrow(sis_no_acad_plan)
```
...
```{r, fig.retina = 2, fig.width = 8, fig.height = 7}
viz_OutcomesByStudentCreditLoad2 <- function(statics, sis_no_acad_plan) {
...
v <- viz_OutcomesByStudentCreditLoad2(statics, sis_no_acad_plan)
v
```
