...
```{r, fig.width = 8, fig.height = 7}
tmp_cor <- cor(statics_w_bio227$NormalizedScore, statics_w_bio227$GradePoints, use = "complete.obs")
tmp_cor

lm_eqn <- function(df){
    m <- lm(NormalizedScore ~ GradePoints, df);
    eq <- substitute(italic(NormalizedScore) == a + b %.% italic(GradePoints)*","~~italic(r)^2~"="~r2, 
...
ggplot(statics_w_bio227, aes(y = NormalizedScore, x = GradePoints)) +
  ...
  my_theme() +
  labs(title = "SU20 BIOL 228 Performance by BIOL 227 Outcome", 
       subtitle = lm_eqn(statics_w_bio227),
  ...
  geom_text(size = 4, x = 200, y = 200, label = lm_eqn(statics_w_bio227), parse = TRUE)
```
