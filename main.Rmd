...
colnames(prerequisites) <- c("TermKey", "TermSourceKey", "SessionCode", "ClassNumberSectionUniqueDescription", "ClassNumberUniqueDescription",
                             ...
                             "CreditsEarned", "EarnCreditIndicator", "SuccessInidicator", "HasClassGrade", "ClassGrade", "GradeKey",
...
p1 <- prerequisites %>%
  filter(EnrolledClassCount == 1,
         DropCount == 0,
         WithdrawCount == 0) %>%
  filter(HasClassGrade == 1) %>%
  select(StudentID, TermSourceKey, PrimarySubject, PrimaryCatalogNumber, ClassSection, GradeLetter, GradePoints, GradeDescription, GradeSubgroup, GradeGroup,
         GradingBasisDescription, ValueFormalDescription)


# Cast data
p2 <- reshape2::dcast(p1, 
                      StudentID + TermSourceKey + PrimarySubject + PrimaryCatalogNumber + ClassSection + GradeLetter + GradePoints + 
