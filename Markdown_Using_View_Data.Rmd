...
```{r}
sis2 <- sis %>%
  mutate(Plan10 = ifelse(AcademicPlan %in% plans_10_or_more$AcademicPlan, AcademicPlan, "Plan w low count")) %>%
  select(Plan10, StudentID_Anon, everything()) %>%
...
```{r}
sis3 <- data.frame(cbind(sis2, dummies::dummy(sis2$Plan10))) # Append dummies for Academic Plan
```

#### Remove Plan column and collapse duplicates

```{r message=FALSE}
columns_for_group <- sis2 %>%
  select(-Plan10)
grp_cols <- names(columns_for_group)
grp_cols

sis4 <- sis3 %>%
  group_by(.dots = grp_cols) %>%
  summarise(across(starts_with("Plan10."), max)) %>%
  ungroup()
```
